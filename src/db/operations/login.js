...
const deleteLoginCreds = async (username) => {
  ...
  return new Promise((resolve, reject) => {
    Login.findOneAndDelete({ username }, (err, res) => {
      if (err) {
        logger.error(
          `'Unable to remove the login credentials for ${username}'`,
          err
        );
        reject(err);
      }
      ...
      reject({
        status: "error",
        message: `"unable to find login creds for ${username}"`,
      });
    });
  });
};
...
async function getLoginResponse(user, resolve, reject) {
  try {
    ...
    if (userDetails && userDetails.records.length === 1) {
      const detail = {};
      detail.username = userDetails.records[0].username;
      detail.email = userDetails.records[0].email;
